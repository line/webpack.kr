...
`[TerserPlugin]` 및 또는`[function (compiler)]`
...
module.exports = {
  optimization: {
    minimizer: [
      new TerserPlugin({
        parallel: true,
        sourceMap: true, // 프로덕션에서 소스 맵을 사용하는 경우 true로 설정해야 합니다.
        terserOptions: {
...
각 모듈에서 사용된 export를 확인합니다. 이는 [`optimization.providedExports`](#optimizationoccurrenceorder)에 의존합니다. `optimization.usedExports`에 의해 수집된 정보는 다른 최적화 또는 코드 생성에 사용됩니다. 즉, 사용하지 않은 export는 생성되지 않으며, 모든 사용이 호환되는 경우 export 이름이 단일 식별자로 변경됩니다.
