...
T> IIFE(즉시 실행 함수 표현식)는 생성된 직후 바로 실행되는 함수입니다. Webpack은 이를 사용하여 코드를 래핑하고 변수 충돌을 방지합니다.

진입 모듈을 IIFE로 감싸야 하는 경우 `optimization.avoidEntryIife`를 사용하여 IIFE 래핑을 피하십시오([JavascriptModulesPlugin](https://github.com/webpack/webpack/blob/main/lib/javascript/JavascriptModulesPlugin.js)에서 `"This entry needs to be wrapped in an IIFE because"`를 검색하십시오). 이 접근 방식은 JavaScript 엔진의 성능을 최적화하고 ESM 라이브러리 빌드 시 트리 셰이킹을 가능하게 합니다.
