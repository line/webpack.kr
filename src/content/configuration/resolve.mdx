...
| --------------------------------------------------- | --------------------------------------- | ----------------------------------------- |
| `esm`, `wasm`, `loaderImport`                       | `["import", "module", "..."]`           | ESM `import` 문, WebAssembly, 로더 임포트 |
| `commonjs`, `amd`, `loader`, `unknown`, `undefined` | `["require", "module", "..."]`          | `require()` 호출, AMD 및 기타 종속성 유형 |
| `worker`                                            | `["worker", "import", "module", "..."]` | `new Worker()` 표현식                     |
| `css-import`                                        | `["webpack", <mode>, "style"]`          | CSS `@import` 문                          |

예를 들어, 프로젝트가 `target: "web"` 및 `mode: "production"`으로 설정되어 있고 파일에서 `import`를 사용하는 경우, 최종적으로 결정되는 조건은 `["import", "module", "webpack", "production", "browser"]`입니다.
