...
변경되지 않은 모듈의 캐시를 계산하고 변경되지 않은 모듈만 참조합니다. 이는 [`cache.type`](#cachetype)이 `'memory'`로 설정된 경우에만 사용할 수 있으며, [`experiments.cacheUnaffected`](/configuration/experiments/#experimentscacheunaffected)가 활성화 되어 있어야 사용할 수 있습니다.
...
변경되지 않은 모듈의 캐시를 계산하고 변경되지 않은 모듈만 참조합니다. 이는 [`cache.type`](#cachetype)이 `'filesystem'`으로 설정된 경우에만 사용할 수 있으며, [`experiments.cacheUnaffected`](/configuration/experiments/#experimentscacheunaffected)가 활성화 되어 있어야 사용할 수 있습니다.
