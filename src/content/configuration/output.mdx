...
webpack 5.108.0부터 해시 다이제스트 인코딩을 `[<hash>:<digest>:<length>]` 형식으로 인라인 설정할 수도 있습니다. 예: `[contenthash:base64:8]`. 다이제스트는 기본적으로 [`output.hashDigest`](#outputhashdigest)(`hex`)로 설정되며, CSS 모듈 [`localIdentName`](/loaders/css-loader/#localidentname) 자리 표시자에서도 동일한 형식이 작동합니다.
...
주입된 `<script>` 태그가 로드되는 방식을 설정하려면 객체로 설정하세요:

| Option          | Description                                                           |
| --------------- | --------------------------------------------------------------------- |
| `scriptLoading` | `'auto'` (기본값), `'defer'`, 또는 `'blocking'`. 아래 값을 참조하세요. |

- `'auto'`: ES 모듈 출력에 대해서는 모듈 스크립트(`type="module"`)를 생성하고, 그렇지 않으면 지연된 스크립트를 생성합니다.
- `'defer'`: 지연된 `<script defer>`를 강제합니다.
- `'blocking'`: 일반 블로킹 `<script>`를 생성합니다.
...
[엔트리 디스크립터](/configuration/entry-context/#entry-descriptor) `html` 옵션을 통해 엔트리별로 재정의할 수 있습니다.
