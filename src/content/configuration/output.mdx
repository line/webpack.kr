...
청크 로드를 위해 webpack에서 사용하는 전역 변수입니다.
...
청크를 로드하는 방법입니다. 기본적으로 포함된 방법은 'jsonp' (웹), 'importScripts' (WebWorker), 'require' (동기 node.js), 'async-node' (비동기 node.js)이고 다른 방법은 플러그인에 의해 추가될 수 있습니다.
...
청크의 포맷입니다. 기본적으로 포함된 포맷은 'array-push' (web/WebWorker), 'commonjs' (node.js)이고 다른 포맷은 플러그인에 의해 추가 될 수 있습니다.
...
엔트리 포인트에서 사용할 수 있는 청크 로딩 타입 목록입니다. webpack에 의해 자동으로 채워집니다. 함수를 entry 옵션으로 사용하고 거기에서 chunkLoading 옵션을 반환할 때만 필요합니다.
...
청크 수준에서 사용할 수있는 substitution입니다.
...
모듈 수준에서 사용할 수있는 substitution입니다.
...
파일 수준에서 사용할 수있는 substitution입니다.
...
URL 수준에서 사용할 수있는 substitution입니다.
...
[`output.filename`](#outputfilename)과 동일하지만 [애셋 모듈](/guides/asset-modules/)입니다.
...
`'amd-require'` 타입을 사용하면 나중에 별도의 호출없이 AMD 디펜던시를 사용할 수 있습니다. `'amd'` 타입과 마찬가지로 이는 webpack 출력이 로드되는 환경에서 사용할 수있는 적절한 [`require` 함수](https://github.com/amdjs/amdjs-api/blob/master/require.md)에 따라 다릅니다.
...
W> [output.library.type: 'assign-properties'`](#type-assign-properties-badge-text5160-)를 사용하는 것이 좋습니다.
...
엔트리 포인트에서 사용할 수있는 라이브러리 타입 목록입니다.
...
module.exports = {
  output: {
    environment: {
      // 화살표 함수('() => { ... }')를 지원합니다.
      arrowFunction: true,
      // BigInt를 123n 같은 리터럴로 지원합니다.
      bigIntLiteral: false,
      // const와 let을 사용한 변수 선언을 지원합니다.
      const: true,
      // '{ a, b } = obj' 같은 구조 분해 할당을 지원합니다.
      destructuring: true,
      // EcmaScript 모듈 import를 위한 비동기 import() 함수를 지원합니다.
      dynamicImport: false,
      // 'for (const x of array) { ... }' 같은 'for of' iteration을 지원합니다.
      forOf: true,
      // import ... from '...' 같은 EcmaScript 모듈 import를 위한 ECMAScript 모듈 구문을 지원합니다.
      module: false,
    },
  },
};
...
엔트리 포인트에서 사용할 수있는 wasm 로딩 유형 목록입니다.
...
module.exports = {
  //...
  output: {
    clean: true, // 방출하기 전에 출력 디렉토리를 정리합니다.
  },
};
```
...
module.exports = {
  ...
  output: {
    clean: {
      keep: /ignored\/dir\//, // 애셋을 'ignored/dir'에 유지시킵니다.
    },
  },
};
