...
T> `worker` 플래그(webpack 5.108.0에 추가됨)는 엔트리를 worker로 표시하여, 출력 파일이 일반 청크 파일명 대신 [`output.workerChunkFilename`](/configuration/output/#outputworkerchunkfilename)을 사용하도록 합니다. webpack은 `new Worker(new URL(...))`에서 생성하는 엔트리에 대해 자동으로 설정하므로, 수동으로 설정하는 경우는 드뭅니다.
