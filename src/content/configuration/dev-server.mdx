...
이 옵션을 사용하면 개발 서버에 접근할 수 있는 서비스를 화이트리스트에 추가할 수 있습니다.
...
장고의 `ALLOWED_HOSTS`를 모방하여, `.`로 시작하는 값을 하위 도메인 와일드카드로 사용할 수 있습니다. `.host.com`은 `host.com`, `www.host.com`, 및 `host.com`의 다른 하위 도메인과 일치합니다.
...
`'all'`로 설정하면 이 옵션은 호스트 검사를 무시합니다. **이것은 권장하지 않습니다** 호스트를 확인하지 않는 앱은 DNS 리바인딩 공격에 취약합니다.
...
`'auto'`로 설정하면 이 옵션은 항상 `localhost`, [`host`](#devserverhost), 및 [`client.webSocketURL.hostname`](#websocketurl)를 허용합니다:
...
[커스텀 옵션](https://github.com/watson/bonjour#initializing)을 bonjour에 전달할 수도 있습니다. 예를 들면:
...
이 옵션을 사용하면 클라이언트에 대한 현재 `devServer` 전송 모드를 개별적으로 선택하거나 커스텀 클라이언트 구현을 제공할 수 있습니다. 이를 통해 브라우저나 다른 클라이언트가 `devServer`와 통신하는 방법을 지정할 수 있습니다.
...
이 옵션을 사용하면 웹 소켓 서버에 대한 URL을 지정할 수 있습니다(개발 서버를 프록시하고 클라이언트 스크립트가 연결 위치를 항상 알지 못하는 경우에 유용합니다).
...
W> 라이브 재로딩은 `web`, `webworker`, `electron-renderer` 및 `node-webkit` 과 같은 웹 관련 [대상](/configuration/target/#string)에서만 작동합니다.
...
이 옵션을 사용하면 디렉터리(기본적으로 '공개' 디렉터리)에서 정적 파일을 제공하기 위한 옵션을 구성할 수 있습니다. 비활성화하려면 `false`로 설정하세요:
...
이 옵션을 사용하면 파일 변경 사항을 감시할 globs/directories/files 목록을 구성할 수 있습니다. 예를 들어:
...
이 옵션을 사용하면 현재 웹 소켓 서버를 선택하거나 커스텀 웹 소켓 서버를 제공할 수 있습니다.
