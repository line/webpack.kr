...
네이티브 HTML module 지원을 활성화합니다. JavaScript에서 `.html` 파일을 import하면 그 안의 tag 참조가 일반 webpack pipeline을 거치며, 오랫동안 `html-loader`가 맡아 왔던 역할을 대체합니다. 이 flag는 `NormalModuleFactory`에 `html` module type을 등록하고 아래에 설명된 HTML 동작을 활성화합니다.
...
W> **이 기능은 experimental하며 부분적으로만 구현되었습니다.** Webpack 5.107은 현재 `html-loader`에 해당하는 부분만 구현합니다. 즉, JS에서 HTML 파일을 import했을 때 그 안의 tag 참조를 webpack pipeline으로 처리합니다. **HTML entry points**(`.html` 파일 자체를 `entry`로 사용하는 경우, 즉 `html-webpack-plugin`에 해당하는 부분)는 **아직 지원되지 않으며**, 다음 minor release에서 지원될 예정입니다. 전체 진행 상황은 이슈 [#536](https://github.com/webpack/webpack/issues/536)에서 추적할 수 있습니다.
...
Inline `<script>` 본문은 [`<script src>`](#script-src-and-link-relmodulepreload)에 사용되는 것과 동일한 entry pipeline을 거칩니다. 각 `<script>` 본문은 자체 webpack entry가 됩니다. classic inline script는 CommonJS로 번들되고, `<script type="module">` 본문은 ESM으로 번들됩니다. 생성된 HTML에서 해당 tag는 생성된 chunk를 가리키는 `<script src="…">`로 다시 작성되며, 본문은 비워집니다.
...
- [`output.module`](/configuration/output/#outputmodule)이 활성화되면 classic inline `<script>` tag는 `<script src>`와 동일하게 자동으로 `type="module"`로 업그레이드됩니다.
- `webpackIgnore`는 inline `<script>` tag에도 동작하며, 이 경우 원래 본문은 변경되지 않습니다.
...
- 같은 페이지의 여러 `<script src>` tag는 하나의 runtime을 공유합니다. 각 그룹(classic 또는 `type="module"`) 내에서 선두 script가 runtime을 보유하고, 나머지는 여기에 `dependOn`을 선언합니다.
- `<link rel="modulepreload">` entry는 독립적으로 유지되며 형제 script에서 import되지 않습니다. 따라서 "preload without execute" 의미가 유지됩니다.
- [`output.module`](/configuration/output/#outputmodule)이 활성화되면 classic `<script src>` tag는 `<script type="module" src>`로 자동 업그레이드되어, 생성된 ES-module chunk가 올바른 mode로 로드됩니다.
- Non-JS script type(`application/ld+json`, `importmap`, …)과 data URI는 변경 없이 그대로 전달되며 JS로 번들되지 않습니다.

#### `webpackIgnore` magic comment

HTML `<!-- webpackIgnore: true -->` comment를 tag 바로 앞에 두면 webpack은 해당 tag의 `src`, `href`, `srcset` 및 유사한 attribute에 대한 URL resolve를 건너뜁니다. 자세한 설명은 [magic comments](/api/module-methods/#webpackignore)를 참고하세요.
