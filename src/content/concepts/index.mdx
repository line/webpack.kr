...
T> 설정 파일 없이 webpack을 실행하면 기본 엔트리포인트는 `./src/index.js`로 설정됩니다. `src/` 디렉터리가 존재하더라도 해당 파일이 없으면 webpack은 다음과 같은 오류를 발생시킵니다. <br /> **`ERROR in Entry module not found: Error: Can't resolve './src'`** <br /><br /> 오류 메시지에 `./src/index.js`가 아닌 `./src`가 표시되는 이유는 webpack이 먼저 디렉터리를 확인한 후 그 안에 있는 기본 `index.js` 파일을 찾지 못했기 때문입니다. 다른 엔트리포인트 파일 이름을 사용하려면 위 예시와 같이 `webpack.config.js` 파일을 추가하세요.
...
import webpack from "webpack"; // 내장 plugin에 접근하는 데 사용
