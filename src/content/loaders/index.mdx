...
Webpack은 [loaders](/concepts/loaders)를 사용하여 파일을 전처리할 수 있도록 지원합니다. 이를 통해 JavaScript뿐만 아니라 거의 모든 정적 리소스를 번들링할 수 있습니다. Node.js를 사용하면 자신만의 로더를 쉽게 작성할 수 있습니다.
로더는 Webpack의 기능을 확장하는 별도의 패키지이며, Webpack 생태계 내에서 관리됩니다.
로더는 `import .. from "mod";`/`require()` 문에서 `loadername!` 접두사를 사용하여 활성화하거나, Webpack 설정에서 정규 표현식을 통해 자동으로 적용됩니다. 자세한 내용은 [설정](/concepts/loaders/#configuration)을 참조하세요.
...
- [`remark-loader`](https://github.com/webpack/remark-loader)는 빌트인 이미지 해상도로 `remark`를 통해 마크다운을 로드합니다.
...
- [`css-utility-loader`](https://github.com/SahilKhanWDC/css-utility-loader-webpack-.git)는 빌드 시점에 기존 CSS 속성을 자동으로 구문 분석하고 최신 유틸리티 클래스(Tailwind)로 변환합니다.
