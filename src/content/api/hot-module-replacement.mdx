...
[Hot Module Replacement](/concepts/hot-module-replacement)가 [`HotModuleReplacementPlugin`](/plugins/hot-module-replacement-plugin)를 통해 활성화 된 경우, 이것의 인터페이스는 [`import.meta.webpackHot`](/api/module-variables/#importmetawebpackhot)프로퍼티 뿐만 아니라 [`module.hot`](/api/module-variables/#modulehot-webpack-specific)프로퍼티 아래 노출됩니다. [strict ESM](/guides/ecma-script-modules/#flagging-modules-as-esm)에서는 `import.meta.webpackHot`만 사용할 수 있습니다.

일반적으로, 사용자는 인터페이스에 접근할 수 있는지 확인한 후, 작업을 시작합니다. 예를들어, 업데이트된 모듈을 `accept` 하는 방법은 다음과 같습니다.
...
CommonJS를 사용할 때 `callback`에서 `require()`를 사용해서 의존성을 수동으로 업데이트 해야 합니다. 여기서 `callback`을 생략하는 것은 의미가 없습니다.

#### errorHandler for accept

`(err, {moduleId, dependencyId}) => {}`

- `err`: ESM 의존성을 사용할 때 두번째 인수 또는 의존성 실행 중에 콜백에 의해 발생한 에러입니다.
- `moduleId`: 현재 모듈 id 입니다.
- `dependencyId`: (첫번째) 변경된 의존성의 모듈 id.
...
```js
module.hot.accept(
  errorHandler // 새 버전을 평가할 떄 에러 처리 기능
);
...
모듈 또는 의존성이 업데이트 되면, 부모에게 알리지 않고 이 모듈을 폐기하고 재평가 할 수 있습니다. 이것은 이 모듈에 내보내기가 없는 경우(또는 내보내기가 다른 방식으로 업데이트 되는 경우) 의미가 있습니다.
...
- `moduleId`: 현재 모듈 id.
- `module`: 현재 모듈 인스턴스.
  - `module.hot`: 에러가 발생한 모듈 인스턴스의 HMR API를 사용하도록 허용합니다. 일반적인 시나리오는 스스로 그것을 다시 받아들이는 것입니다. 또한 데이터를 전달하기 위해 dispose 핸들러를 추가하는 것이 좋습니다. 에러가 발생한 모듈은 이미 부분적으로 실행되었을 수 있으므로, 일관성 없는 상태가 되지 않도록 주의하세요. `module.hot.data`를 사용하여 부분 상태를 저장할 수 있습니다.
  - `module.exports`: 오버라이드할 수 있지만, 프로덕션 모드에서 프로퍼티 이름이 손상될 수 있으므로 주의하세요.
...
업데이트 불가능으로 모듈에 플래그를 지정합니다. 이는 이 모듈에 되돌릴 수 없는 사이드 이팩트가 있거나, 이 모듈에 대해 HMR 핸들링이 아직 구현되지 않은 경우 의미가 있습니다. HMR 관리 코드에 따라, 이러한 모듈(또는 허용되지 않은 의존성)에 대한 업데이트로 인해 일반적으로 전체 페이지가 다시 로드됩니다.

### `dispose` (또는 `addDisposeHandler`)

현재 모듈 코드가 교체될 때 실해오디는 핸들러를 추가합니다. 이것은 요청하거나 생성한 영구 리소스를 제거하는데 사용해야 합니다. 업데이트 된 모둘에 상태를 전송하려면, 주어진  `data` 파라미터에 추가하세요. 이 객체는 업데이트 후 `module.hot.data`에서 사용할 수 있습니다.

```js
module.hot.dispose((data) => {
  // 업데이트된 모듈에 데이터를 정리하고 전달...
});
...
이 메서드를 호출하면 현재 모듈이 무효화되고, HMR 업데이트가 적용될 때 이를 삭제하고 다시 생성합니다. 이것은 이 모듈의 일반적 업데이트와 같은 버블입니다. 이 모듈은 `invalidate`를 자체 승인할 수 없습니다.

`idle` 상태에서 호출되면, 이 모듈을 포함하는 새 HMR 업데이트가 생성됩니다. HMR은 `ready` 상태로 들어갑니다.

`ready` 또는 `prepare` 상태에서 호출되면, 이 모듈은 현재 HMR 업데이트에 추가됩니다.

`check` 상태에서 호출되면, 이 모듈은 업데이트를 사용할 수 있을 때 업데이트에 추가됩니다. 사용가능한 업데이트가 없으면 새 업데이트가 생성됩니다. HMR은 `ready` 상태로 들어갑니다.
...
모듈은 자체 수용할 수 있지만, 변경 사항을 처리할 수 없을 때 자체를 무효화할 수 있습니다.
...
T> `invalidate`가 호출되면, `dispose`](#dispose-or-adddisposehandler) 핸들러가 결국 호출되어 `module.hot.data`를 채웁니다. [`dispose`](#dispose-or-adddisposehandler) 핸들러가 등록되지 않은 경우, `module.hot.data`에 빈 객체가 제공됩니다.

W> `invalidate`를 계속 호출하여, `invalidate` 루프에 걸리지 마세요. 이로인해 스택 오버플로우가 발생하고 HMR이 `fail` 상태가 됩니다.
...
| prepare | 프로세스가 업데이트를 준비중입니다 (예. 업데이트 된 모듈 다운로드)                                  |
...
`autoApply` 파라미터는 boolean이거나 호출될 때 `apply`메서드에 전달 할 `options`일 수 있습니다.
...
상태 핸들러가 `Promise`를 반환하면, HMR 시스템은 계속하기 전에 `Promise`가 해결될 때 까지 기다립니다.
