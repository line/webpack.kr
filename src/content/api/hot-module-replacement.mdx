...
[Hot Module Replacement](/concepts/hot-module-replacement)가 [`HotModuleReplacementPlugin`](/plugins/hot-module-replacement-plugin)를 통해 활성화된 경우, 이것의 인터페이스는 [`import.meta.webpackHot`](/api/module-variables/#importmetawebpackhot)프로퍼티 뿐만 아니라 [`module.hot`](/api/module-variables/#modulehot-webpack-specific)프로퍼티 아래 노출됩니다. [strict ESM](/guides/ecma-script-modules/#flagging-modules-as-esm)에서는 `import.meta.webpackHot`만 사용할 수 있습니다.
...
CommonJS를 사용할 때 `callback`에서 `require()`를 사용해서 의존성을 수동으로 업데이트해야 합니다. 여기서 `callback`을 생략하는 것은 의미가 없습니다.
...
```js
module.hot.accept(
  errorHandler // 새 버전을 평가할 때 에러 처리 기능
);
...
- `moduleId`: 현재 모듈 id.
- `module`: 현재 모듈 인스턴스.
  - `module.hot`: 에러가 발생한 모듈 인스턴스의 HMR API를 사용하도록 허용합니다. 일반적인 시나리오는 스스로 그것을 다시 받아들이는 것입니다. 또한 데이터를 전달하기 위해 dispose 핸들러를 추가하는 것이 좋습니다. 에러가 발생한 모듈은 이미 부분적으로 실행되었을 수 있음으로, 일관성 없는 상태가 되지 않도록 주의하세요. `module.hot.data`를 사용하여 부분 상태를 저장할 수 있습니다.
...
현재 모듈 코드가 교체될 때 실행되는 핸들러를 추가합니다. 이것은 요청하거나 생성한 영구 리소스를 제거하는 데 사용해야 합니다. 업데이트된 모둘에 상태를 전송하려면, 주어진  `data` 파라미터에 추가하세요. 이 객체는 업데이트 후 `module.hot.data`에서 사용할 수 있습니다.
...
이 메서드를 호출하면 현재 모듈이 무효화 되고, HMR 업데이트가 적용될 때 이를 삭제하고 다시 생성합니다. 이것은 이 모듈의 일반적 업데이트와 같은 버블입니다. 이 모듈은 `invalidate`를 자체 승인할 수 없습니다.
...
W> `invalidate`를 계속 호출하여, `invalidate` 루프에 걸리지 마세요. 이로 인해 스택 오버플로우가 발생하고 HMR이 `fail` 상태가 됩니다.
