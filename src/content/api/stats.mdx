...
webpack으로 소스 코드를 컴파일 할때, 사용자는 모듈에 대한 통계가 포함된 JSON 파일을 생성할 수 있습니다. 이러한 통계는 애플리케이션의 디펜던시 그래프를 분석하고 컴파일 속도를 최적화하는데 사용할 수 있습니다. 파일은 일반적으로 다음 CLI 커맨드로 생성됩니다.
...
출력 JSON 파일의 최상위 구조는 매우 간단하지만 몇가지 중첩된 데이터 구조도 있습니다. 각 중첩 구조에는 이 문서를 보다 유용하게 사용할 수 있도록 아래에 전용 섹션이 있습니다. 아래의 최상위 구조 내의 링크를 클릭하면 관련 섹션 및 문서로 이동할 수 있습니다.
...
```js
{
  ...
  'size': 1058, // 파일 크기(바이트)
  'info': {
    'immutable': true, // 애셋을 장기간 캐시할 수 있는지 여부를 나타내는 플래그(해시 포함)
    'size': 1058, // 바이트 단위 크기, 애셋을 내보낸 뒤에만 사용할 수 있습니다
...
T> 애셋의 `info` 프로퍼티는 webpack v4.40.0 부터 사용할 수 있습니다.

### Chunk Objects

각 `chunks`객체는 [chunk](/glossary/#c)로 알려진 모듈 그룹을 나타냅니다. 각 객체는 다음의 구조를 따릅니다.
...
```js-with-links
{
  ...
  "id": 0, // 이 청크의 ID
  "initial": true, // 이 청크가 초기 페이지 로드시 로드되는지 아니면 [on demand](/guides/lazy-loading)시 로드되는지 여부
  "modules": [
    // [module objects](#module-objects) 목록
    "web.js?h=11593e3b3ac85436984a"
  ],
  "names": [
    // 이 청크에 포함된 청크 이름 목록
...
{
  "assets": [
    // [asset objects](#asset-objects) 목록
  ],
  "built": true, // 모듈이 [Loaders](/concepts/loaders), 파싱, 및 코드 제너레이션을 거쳤음을 의미
  ...
  "warnings": 0 // 모듈을 리졸브하거나 처리할 때 경고의 수
}
```

모든 모듈에는 해당 모듈이 의존성 그래프에 포함된 이유를 설명하는 `reasons`객체 목록도 포함되어 있습니다. 각 "reason"는 위의 [chunk objects](#chunk-objects)섹션에서 본 `origins`와 유사합니다.
...
```js-with-links
{
  ...
  "moduleId": 0, // 모듈 ID
  "moduleIdentifier": "(webpack)\\test\\browsertest\\lib\\index.web.js", // 모듈 경로
  "moduleName": "./lib/index.web.js", // 모듈의 더 읽기 쉬운 이름 ("pretty-printing"에 사용 됨)
