...
`--define-process-env-node-env` 옵션을 사용하여 `process.env.NODE_ENV`를 설정할 수 있습니다.

```bash
npx webpack --define-process-env-node-env production   # process.env.NODE_ENV = 'production'
```

T> `mode` 옵션을 명시적으로 설정하지 않는 경우 `--define-process-env-node-env` 옵션을 따릅니다. 예를들어, `--define-process-env-node-env production`과 같이 설정하면 ` process.env.NODE_ENV` 와 `mode`는 둘 다 `'production'`으로 설정됩니다.
