...
[`experiments.html`](/configuration/experiments/#experimentshtml)가 활성화되면, `webpackIgnore`를 tag 바로 앞의 HTML comment로 배치하여 해당 tag의 `src`, `href`, `srcset` 및 유사한 attribute에 대한 URL resolve를 건너뛸 수 있습니다. 이 경우 생성된 HTML에서 해당 tag는 수정되지 않은 채 유지됩니다. 이는 `html-loader`가 제공하던 동작과 동일합니다.
