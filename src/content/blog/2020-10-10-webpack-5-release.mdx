...
실링 프로세스의 여러 단계에 대해 여러개의 후크가 있었습니다. 즉 `optimizeDependenciesBasic` `optimizeDependencies` 및 `optimizeDependenciesAdvanced`는 `stage` 옵션과 함께 사용할 수 있는 단일 후크를 위해 제거되었습니다. 가능한 단계 값은 `OptimizationStages`를 참고하세요.
