...
기본적으로 webpack은 webpack이 있는 `node_modules` 디렉토리가 **오직** 패키지 관리자에 의해 수정된다고 가정합니다. `node_modules`에 대해 해싱 및 타임스탬프를 건너뜁니다.
...
- 기본 Node.js 모듈에 대한 자동 폴리필이 제거되었습니다.
  ...
  - **마이그레이션**: `resolve.alias` 및 `ProvidePlugin`. 오류가 힌트를 줄것입니다. (v4에서 사용되는 폴리필 및 참조는 [node-libs-browser](https://github.com/webpack/node-libs-browser) 참조)
