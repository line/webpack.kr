...
Webpack 프로젝트를 빠르게 구축하고 실행하고 싶다면 `create-webpack-app`을 사용하여 스캐폴딩하는 것이 가장 쉬운 방법입니다.
...
또한 `package.json` 파일에서 패키지를 `private`으로 표시하고 `main` 항목을 삭제해야 합니다. Node.js가 설정 파일을 ES 모듈로 처리하도록 `type: module`도 추가합니다. 이는 코드가 실수로 공개되는 것을 방지하고 스크립트에서 ES 모듈을 지원하도록 하기 위함입니다.
