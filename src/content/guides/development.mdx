...
T> `optimization.runtimeChunk: 'single'`이 추가된 이유는 이 예제에서 단일 HTML 페이지에 하나 이상의 엔트리포인트가 있기 때문입니다. 이것이 없으면 [여기에](https://bundlers.tooling.report/code-splitting/multi-entry/) 설명된 문제가 발생할 수 있습니다. 자세한 내용은 [코드 스플리팅](/guides/code-splitting/) 내용을 참고하세요.
