...
반면, `index.js`는 `printMe`를 가져와 사용하므로 webpack은 `index.bundle.js`에 해당 함수를 직접 포함합니다. 웹팩은 모듈 번들러이므로 `index.bundle.js` 파일에는 해결되지 않은 import 문 대신 실행에 필요한 모든 코드가 포함되어 있다는 점을 기억하세요.<br /><br />
