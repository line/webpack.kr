...
ESM의 import는 더 엄격하게 해석됩니다. 상대적 요청에는 [`fullySpecified=false`](/configuration/module/#resolvefullyspecified)로 동작을 비활성화하지 않는 한 파일 이름과 파일 확장자가 포함되어야 합니다 (예를 들면 `*.js` 또는 `*.mjs`).
