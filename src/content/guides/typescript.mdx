...
`webpack.config.ts`에서 TypeScript를 사용하는 방법은 5가지가 있습니다.

1. **TypeScript 구성과 함께 webpack 사용**

   ```bash
   webpack -c ./webpack.config.ts
   ```

   (`rechoir`와 `interpret`의 한계로 인해 모든 기능이 지원되지는 않습니다.)

2. **Node.js에 사용자 정의 `--import` 사용:**

   ```bash
   NODE_OPTIONS='--import tsx'  webpack --disable-interpret -c ./webpack.config.ts
   ```

3. **Node.js v22.7.0 ≥ 당신의 NODE.JS 버전 < v23.6.0에 내장된 TypeScript 모듈 사용**

   ```bash
   NODE_OPTIONS='--experimental-strip-types' webpack --disable-interpret -c ./webpack.config.ts
   ```

4. **Node.js에 내장된 TypeScript 모듈 사용 ≥ v22.6.0:**

   ```bash
   webpack --disable-interpret -c ./webpack.config.ts
   ```

5. **Node.js ≥ v22.6.0에 tsx 사용**
