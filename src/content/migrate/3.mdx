...
T> webpack 2와 3 사이에 변경 사항이 훨씬 적으므로 이주가 그렇게 나쁘지 않을 것입니다. 문제가 발생하는 경우, 자세한 내용은 [the changelog](https://github.com/webpack/webpack/releases)를 참고하세요.

## resolve.root, resolve.fallback, resolve.modulesDirectories

위 3개의 파일들 `resolve.modules`하나로 대체되었습니다. 자세한 사용법은 [resolving](/configuration/resolve)을 참고하세요.
...
이 옵션은 더 이상 빈 문자열을 전달할 필요가 없습니다. 이 동작은 `resolve.enforceExtension`로 이동되었습니다. 자세한 사용법은 [resolving](/configuration/resolve) 를 참고하십시오.
...
이전의 로더 은 로더와 그 이상의 설정이 허가되는 강력한규칙 시스템으로 대체 되었습니다.
호환성을 위해 이전의 `module.loaders` 구문은 여전히 유효하며 이전 이름은 구문 분석됩니다.
네이밍 들이 이해하기 쉬워졌고 `module.rules`를 사용하도록 설정을 업그레이드 해야하는 좋은 이유입니다.
...
JSON 파일에 대해 설정된 로더가 없으면, webpack은 자동으로 JSON [`json-loader`](https://github.com/webpack-contrib/json-loader) 파일을 로드하려고 시도합니다.
...
**webpack 1** 에서, 된 로더가 일치하는 파일을 기준으로 확인되어집니다. 하지만, **webpack 2** 에서는, 설정된 로더는 `context` 옵션을 기준으로 확인됩니다.
...
`UglifyJsPlugin` 의 `sourceMap` 의 옵션은 `true` 대신 `false` 로 기본 설정됩니다. 즉, 최소화된 코드에 소스맵을 사용하거나 uglifyjs 경고에 대한 올바른 줄 번호를 원하는 경우 `UglifyJsPlugin`에서 `sourceMap: true`를 설정해야 합니다.
...
`webpack.optimize.DedupePlugin`은 더 이상 필요하지 않습니다. 설정에서 제거하십시오.
...
`webpack.config.js`에서 사용자 정의 속성으로 로더를 설정할 수 _더 이상_ 없습니다. `options`을 통해 수행해야 합니다.`ts` 속성이 있는 다음 설정은 더 이상 webpack 2에서 유효하지 않습니다 :
...
좋은 질문입니다. 음, 엄밀히 말하면 2가지 가능성이 있습니다. webpack 로더를 설정하는 두 가지 방법. 고전적으로 `options`은 `query`라고 했으며 로더 이름에 추가할 수 있는 문자열이었습니다. 쿼리 문자열과 매우 유사하지만 실제로는 [더 강력한 기능](https://github.com/webpack/loader-utils#parsequery)입니다:
...
일부 로더는 컨텍스트 정보가 필요하고 설정에서 이를 읽습니다. 이것은 장기적으로 로더 옵션을 통해 전달되어야 합니다. 관련 옵션은 로더 문서를 참고하십시오.
...
`debug` 옵션은 webpack 1에서 로더를 디버그 모드로 전환했습니다. 이것은 장기적으로 로더 옵션을 통해 전달되어야 합니다. 관련 옵션은 로더 문서를 참고하십시오.
...
webpack 1에서는 [`require.ensure()`](/api/module-methods/#requireensure) 를 응용 프로그램의 를 지연 로드하는 방법으로 사용할 수 있습니다 :
