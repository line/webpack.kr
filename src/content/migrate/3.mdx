...
이 옵션은 더 이상 빈 문자열을 전달할 필요가 없습니다. 이 동작은 `resolve.enforceExtension`로 이동되었습니다. 자세한 사용법은 [resolving](/configuration/resolve) 를 참고하세요.
...
[환경차이를 없애기 위해 그렇게 하기로 결정했습니다.](https://github.com/webpack/webpack/issues/3363) webpack, node.js 및 browserify 간의 환경차이를 없애기 위해서입니다.
...
`--define process.env.NODE_ENV="production"` 는 [`DefinePlugin`](/plugins/define-plugin/)를 참고하세요.
