...
- webpack 설정에서 `optimization.moduleIds` 및 `optimization.chunkIds` 제거를 고려하세요. 기본값은 [`production mode`](/configuration/mode/#mode-production)에서 장기 캐싱을 지원하고 [`development` mode](/configuration/mode/#mode-development)에서 디버깅을 지원하기 때문에 더 나을 수 있습니다.
...
- `Optimization.splitChunks.cacheGroups: { default: false, vendors: false }`를 설정하여 기본값을 끌(turn off) 수 있었습니다. 이것을 권장하지 않지만, webpack 5에서 같은 효과를 얻고 싶다면 `optimization.splitChunks.cacheGroups: { default: false, defaultVendors: false }`를 사용할 수 있습니다.
...
빌드 errors/warnings를 주의 깊게 읽어보세요. 해당하는 조언이 없는 경우 issue를 생성해 주시면 해결해 드리겠습니다.
