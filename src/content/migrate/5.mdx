...
webpack 5의 구성을 업그레이드할 때 이 옵션을 다시 제거해야 합니다.

T> webpack 5는 구성 스키마에서 이러한 옵션을 제거하고 항상 `false`를 사용합니다.
...
- webpack 구성에서 `optimization.moduleIds` 및 `optimization.chunkIds` 제거를 고려하세요. 기본값은 [`production mode`](/configuration/mode/#mode-production)에서 장기 캐싱을 지원하고 [`development` mode](/configuration/mode/#mode-development)에서 디버깅을 지원하기 때문에 더 나을 수 있습니다.
...
- 사용자 정의 구성을 사용할 때 `name: false`를 삭제하고 `name: string | function`을 `idHint: string | function`로 교체하세요.
...
- 프로젝트에 대해 [browserslist](https://github.com/browserslist/browserslist)를 활성화한 경우 webpack 5는 `browserslist` 구성을 재사용하여 런타임 코드에 대해 내보낼 코드 스타일을 결정합니다.

  확실히 할 것들:

  1. [`target`](/configuration/target/#root)을 `browserslist`으로 설정하거나 webpack이 자동으로 `browserslist`을 설정하도록 `target`을 제거합니다.
  2. `IE 11`을 browserslist 구성에 추가합니다.
