...
3. 사용한 모든 플러그인과 로더를 사용 가능한 최신 버전으로 업그레이드하세요.
   ...
   최신 버전이 webpack 5만 지원할 수 있고, 이때 v4에서는 실패할 수 있으므로 업그레이드할 때는 각 개별 플러그인과 로더의 릴리스 정보를 읽어보세요. 이 경우 webpack 4를 지원하는 최신 버전으로 업데이트 하는것을 권장합니다.
...
Webpack 4 및 해당 플러그인/로더 업그레이드 단계에서 일부 플러그인/로더를 최신 버전으로 업그레이드할 수 없는 경우, 지금 잊지말고 업그레이드하세요.
