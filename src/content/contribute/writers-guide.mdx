...
- `title`: 이 글의 제목입니다.
- `group`: 하위 섹션의 제목입니다.
- `sort`: 해당 섹션 (또는) 하위 섹션 내에서 이 글의 순서입니다.
- `contributors`: 이 글에 기여한 사람들이 GitHub username 리스트입니다.
- `related`: 관련된 읽을거리 또는 유용한 예시입니다.

`related`는 페이지 하단에 **추가 읽기** 섹션을 생성하고 `contributors`는 그 아래에 **Contributors** 섹션을 생성합니다. 글을 편집하고 인정받고 싶다면 주저하지 말고 GitHub 사용자 이름을 'contributors' 목록에 추가하세요.

## Article Structure

1. 간략한 소개 - 어떤 것을 왜 하였는지에 대한 아이디어를 적은 한두 문장.
2. 남은 콘텐츠 개요 – 콘텐츠가 표시되는 방식.
...
- 로더는 역 따옴표로 묶여 있으며 [kebab-cased](https://en.wikipedia.org/w/index.php?title=Kebab_case): `css-loader`, `ts-loader`, …
- 플러그인은 백틱 및 [camel-cased](https://en.wikipedia.org/wiki/Camel_case)로 묶입니다: `BannerPlugin`, `NpmInstallWebpackPlugin`, …
...
| --devtool  | 번들 리소스에 대한 소스 맵 유형 정의 | string     | -             |
...
모든 것에 단순하다고 가정하지 마세요. 'just', 'simply'와 같은 단어를 피하세요.
...
문서를 액세스할 수 있고 잘 작성할 수 있도록 항상 모든 문서 옵션에 유형과 기본값을 제공하세요. 저희는 문서화된 옵션에 권한을 부여한 후 유형과 기본값을 추가합니다.
...
객체의 키가 동적이며 사용자 정의된 경우 ``를 사용하여 설명합니다.
...
예제는 [EvalSourceMapDevToolPlugin' page의 `options` 섹션](/plugins/eval-source-map-dev-tool-plugin/#options)에서 찾을 수 있습니다.
