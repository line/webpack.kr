...
플러그인은 서드파티 개발자들에게 webpck 엔진의 모든 잠재력을 보여준다. 단계별로 빌드 callback을 사용하여 개발자들은 webpack의 빌드 프로세스에 자신만의 행동을 도입할 수 있습니다. 플러그인을 빌드하는 것은 로더를 빌딩 하는 것보다 조금 더 진보되었습니다. 왜냐하면, 플러그인은 몇몇 webpack의 low-level에 내장된 훅을 이해할 필요가 있기 때문입니다. 이제 소스 코드를 읽어볼 준비를 해봅시다!
...
class HelloCompilationPlugin {
  apply(compiler) {
    // callback 함수에 argument로 compilation을 제공하는 훅으로 compliation 훅을 탭 합니다.
    compiler.hooks.compilation.tap('HelloCompilationPlugin', (compilation) => {
      //이제 우리는 컴파일을 통해 이용할 수 있는 다양한 훅들을 이용할 수 있습니다.
...
`compiler`, `compilation` 그리고 다른 중요한 객체에서 사용 가능한 훅들은 [plugins API](/api/plugins/) 문서를 참고해 주세요.
...
T> 위의 예에서 비동기식 동작으로 수행할 필요가 없기 때문에 동기식 `tap()` 메소드로 `processAssets`을 탭 합니다. 그러나 `processAssets` 훅은 비동기식이므로 필요하다면 `tapPromise()` 또는 `tapAsync()`를 사용할 수도 있습니다.
