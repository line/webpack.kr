...
단일 로더를 테스트하기 위해, rule object 내의 로컬 파일을 `resolve`하기 위해 `path`를 사용할 수 있습니다. 
...
다중 로더를 테스트하기 위해, webpack이 로더를 검색할 위치를 업데이트하기 위해 `resolveLoader.modules` 설정을 활용할 수 있습니다. 예를 들어, 프로젝트에서 로컬 `/loaders` 디렉터리를 생성했다고 가정해봅시다.:
...
[`loader-utils`](https://github.com/webpack/loader-utils) 패키지를 활용하세요. 다양한 유용한 도구를 제공하지만 가장 흔한 것 중 하나는 로더로 통하는 옵션을 검색하는 것입니다. `loader-utils`과 함께, [`schema-utils`](https://github.com/webpack-contrib/schema-utils) 패키지는 로더 옵션 유효성에 기반한 일관된 JSON 스키마를 위해 사용돼야 합니다. 여기 둘 다 활용하는 간단한 예시가 있습니다:
...
`less-loader`의 예시에서, 모든 `.less`파일은 변수와 믹스인 추정을 위해 반드시 한 번에 컴파일되어야 하기 때문에 각 `@import`를 `require`로 바꿀 수 없습니다. 그러므로, `less-loader`는 사용자 경로 해석 로직으로 더 적은 컴파일러를 확장합니다. 그런 다음 두 번째 접근 방식인 `this.resolve`를 활용하여 webpack을 통해 의존성을 해결할 수 있습니다. 
