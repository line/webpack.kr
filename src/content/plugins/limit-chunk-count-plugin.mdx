---
title: LimitChunkCountPlugin
group: webpack
contributors:
  - rouzbeh84
  - skipjack
  - tbroadley
  - byzyk
  - EugeneHlushko
  - erykpiast
translator:
  - jiwoo0629
---

코드를 작성할 때, 당신은 이미 필요한 것을 로드할 많은 코드 분할 지점을 추가했을 것입니다. 더 큰 HTTP 오버헤드를 만들면서, 컴파일 후에 어떤 청크가 너무 작다는 것을 깨달을 것입니다. 
`LimitChunkCountPlugin`는 청크를 합침으로써 그것들을 후속 처리할 수 있게 해줍니다.

```js
new webpack.optimize.LimitChunkCountPlugin({
  // Options...
});
```

## Options

다음의 옵션들이 지원됩니다.

### maxChunks

`개수`

청크의 최대 개수를 `1`보다 크거나 같도록 제한합니다. `1`을 사용하는 것은 입력/메인 청크가 카운트에 포함될 때 추가적인 청크가 더해지는 것을 방지합니다.

**webpack.config.js**

```javascript
const webpack = require('webpack');
module.exports = {
  // ...
  plugins: [
    new webpack.optimize.LimitChunkCountPlugin({
      maxChunks: 5,
    }),
  ],
};
```

### minChunkSize

위의 지정된 청크 크기를 유지하는 것은 이 플러그인에서 구현하지 않습니다. [MinChunkSizePlugin](/plugins/min-chunk-size-plugin)를 대신 사용하세요.

## Usage via CLI

이 플러그인과 옵션은 [CLI](/api/cli/)를 통해 호출될 수도 있습니다.


```bash
webpack --optimize-max-chunks 15
```
