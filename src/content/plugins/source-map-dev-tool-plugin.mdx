...
- `append` (`string` `function`): 기존 애셋에 값을 추가합니다. 일반적으로 `#sourceMappingURL` 주석이 사용됩니다. `[url]`은 소스맵 파일의 URL로 대체됩니다. Webpack v4.36.0부터는 `[chunk]`, `[filename]` 및 `[contenthash]`와 같은 경로 파라미터가 지원됩니다. `false`로 설정하면 값 추가가 비활성화됩니다.
