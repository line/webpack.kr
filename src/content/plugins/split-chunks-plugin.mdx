...
T> `splitChunks.cacheGroups.{cacheGroup}.name`은 소스청크의 부모인 청크로 모듈을 이용하는 데 사용할 수 있습니다. 예를 들어 `name: "entry-name"`을 사용하여 모듈을 `entry-name` 청크로 이동합니다. 요청 시 명명된 청크를 사용할 수도 있지만 선택한 모듈이 청크 아래에사만 사용된다는 점에 주의해야 합니다.
