...
_Context는_ `require('./locale/' + name + '.json')`와 같은 [require with an expression](/guides/dependency-management/#require-with-expression)을 나타냅니다. 이러한 표현식을 만나면, webpack은 디렉터리 (`'./locale/'`)와 정규 표현식 (`/^.*\.json$/`)을 추론합니다. 컴파일 시 `name`을 알 수 없기 때문에 webpack은 모든 파일을 번들에 모듈로 포함합니다.
