...
- `progressBar` (`boolean | object = false`) <Badge text="5.107.0+" />: 단순한 percentage 메세지 대신 터미널에 progress bar를 렌더링합니다. 기본 handler를 사용할 때만 적용되며, 즉 사용자 지정 `handler`를 제공하지 않았을 때만 동작합니다. 기본값으로 활성화하려면 `true`를 전달하고, 아래 속성을 가진 object를 전달할 수도 있습니다.
