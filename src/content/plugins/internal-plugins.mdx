...
W> Webpack 기반으로 나만의 컴파일러를 구축하거나, 내부 검토할 때만 신경써야합니다.
...
컴파일러에 훅을 하여 진행 정보들을 추출합니다. `handler`는 `function(percentage, message)`라는 서명이 있어야 합니다. Percentage는 0과 1 사이의 값으로 불립니다. 0은 시작, 1은 끝을 나타냅니다.

### RecordIdsPlugin

`RecordIdsPlugin()`

레코드로부터 청크 아이디들과 모듈을 저장하고 복원합니다.
...
컴파일에 엔트리 청크를 추가합니다. 청크의 이름은 `options.name`이고 오직 하나 모듈만 포함합니다(의존성 추가). 모듈은 `context` (절대 경로)의 `entry`에서 확인됩니다.
...
`options.chunkFilename`는 청크 아래에 있는 파일 이름으로 예상됩니다.

### NodeTemplatePlugin

`node/NodeTemplatePlugin(options)`

청크는 번들 모듈들을 내보내는 Node.js 모듈들로 래핑되어 있습니다. 엔트리 청크는 청크를 요구함으로써 로드됩니다.
...
엔트리 청그들은 `type` 유형의 `name` 라이브러리로부터 꾸며집니다.
...
각 청크에 대한 SourceMap을 생성함으로써 템플릿들을 꾸밉니다.

`sourceMapFilename`은 SourceMap의 파일 이름 템플릿입니다. `[hash]`, `[name]`, `[id]`, `[file]` 와 `[filebase]` 는 교체되었습니다. 만약에 인자가 없으면, SourceMap은 DataUrl로 인라인 될 것입니다.

### HotModuleReplacementPlugin

`HotModuleReplacementPlugin(options)`

hot 모듈 교체에 대한 지원을 추가합니다. 런타임 코드를 추가한 템플릿을 꾸밉니다. `module.hot' API를 추가합니다.

`options.hotUpdateChunkFilename`는 hot 업데이트 청크 파일 이름입니다.

`options.hotUpdateMainFilename`는 hot 업데이트 매니페스트 파일 이름입니다.

`options.hotUpdateFunction`는 hot 업데이트의 JSON 함수 이름입니다.

## source

모듈의 소스 코드에 영향을 미치는 플러그인들입니다.

### APIPlugin

webpack_public_path, webpack_require, webpack_modules 과 webpack_chunk_load에 접근할 수 있도록 설정합니다. `require.valueOf`와 `require.onError`가 다른 플러그인들로 처리되지 않도록 합니다.

### CompatibilityPlugin

현재는 쓸모가 없습니다. 다른 모듈 로더들과의 호환성을 보장합니다.

### ConstPlugin

파서에 의해 발사된 훅을 사용함으로써 dead branch를 추가로 제거를 가능하게 하려고 `if (...)`문과 삼항 연산자를 평가하여 `true/false`로 바꾸려고 시도합니다. 

dead branch와 관련하여 프로덕션 모드에는 여러 최적화 들이 있습니다.

- [Terser](https://github.com/fabiosantoscode/terser) 이 수행한 것들입니다.
- Webpack이 수행한 것들입니다.
...
위의 예시에서, Webpack은 branch를 자를 수 없지만, Terser는 할 수 있습니다. 그러나 만약 `FOO`가 [DefinePlugin](/plugins/define-plugin/) 를 사용하면, Webpack은 성공할 것입니다.

`import { calculateTax } from './tax';`도 제거된다는 점을 언급해야 합니다. 왜냐하면 `calculateTax()` 호출이 dead branch에 있어 제거되었기 때문이다.
...
`require[js].config`는 제거됩니다. `require.version`는 `0.0.0`입니다. `requirejs.onError`눈 `require.onError`에 매핑이 됩니다.
...
각 청크에 대한 오버해드는 `options.chunkOverhead` 또는 기본값인 10000에 의해 제공됩니다. 엔트리 청크의 크기는 `options.entryChunkMultiplicator` (또는 10)에 의해 곱해집니다.
...
[ModuleConcatenationPlugin page](/plugins/module-concatenation-plugin/)에 대한 구체적인 내용을 보여줍니다.
...
[`optimization.realContentHash`](/configuration/optimization/#optimizationrealcontenthash) 옵션이 활성화 될 때, Webpac은 내부적으로 `RealContentHashPlugin`을 컴파일러에 추가합니다.

#### Hook

`RealContentHashPlugin`은 제공합니다. 해시 업데이트를 커스터마이징할 수 있는 훅인 `updateHash` <Badge text='5.8.0+' />를 제공합니다.
