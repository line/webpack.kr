...
webpack_public_path, webpack_require, webpack_modules 과 webpack_chunk_load에 접근할 수 있도록 설정합니다. `require.valueOf`와 `require.onError`가 다른 플러그인로 처리되지 않도록 합니다.
...
`require[js].config`는 제거됩니다. `require.version`는 `0.0.0`입니다. `requirejs.onError`는 `require.onError`에 매핑이 됩니다.
